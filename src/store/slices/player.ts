...
const playerSlice = createSlice({
  ...
  reducers: {
    ...
    next: (state) => {
      const nextLessonIndex = state.currentLessonsIndex + 1
      const nextLesson = state.course.modules[state.currentModuleIndex].lessons[nextLessonIndex]

      if (nextLesson) {
        state.currentLessonsIndex = nextLessonIndex
      } else {
...
export const useCurrentLesson = () => {
  return useAppSelector(state => {
    const { currentLessonsIndex, currentModuleIndex } = state.player
    const currentModule = state.player.course.modules[currentModuleIndex]

    const currentLesson = currentModule.lessons[currentLessonsIndex]
